...
class CommandeController extends AbstractController
{
    ...
    public function show1(Request $request,CommandeRepository $commandeRepository,SessionInterface $session): Response
    {
        ...
        $rescommede = $commandeRepository->findBy(['mail' => $email]);
    ...
    public function ajouterproduit(EntityManagerInterface $em, Request $req, PanierServices $panierServices,SessionInterface $session)
    {
        ...
        for ($i = 0; $i < $numberOfDigits; $i++) {
            $randomCombination .= random_int(0, 9);
        }
        $resultString = 'Commande N° ' . $randomCombination;
        $mymail = $session->get('user_email');
    
        $form = $this->createForm(AjoutercommandeType::class, $commande);
        $form->handleRequest($req);
       
        if ($form->isSubmitted() && $form->isValid()) {
            $commande->setIdcommande($resultString);
            $commande->setMail($mymail);
            $em->persist($commande);
            $em->flush();
